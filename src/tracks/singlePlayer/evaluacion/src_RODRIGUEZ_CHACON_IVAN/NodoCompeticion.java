...
public class NodoCompeticion implements Comparable<NodoCompeticion> {
    ...
    public boolean equals(Object o) {
        ...
        return (this.pos.x == n.pos.x && this.pos.y == n.pos.y && this.gemas_capturadas.equals(n.gemas_capturadas) && this.vista == n.vista);
    }
    @Override
    public int hashCode() {
    ...
    private Pair selectBestGem(Pair start, List<Pair> allGems) {
        ArrayList<Pair> gemas = new ArrayList<>(allGems);
        gemas.removeAll(this.gemas_capturadas);
        return gemas.stream()
                    .sorted(Comparator.comparingInt(gem -> 
                        Math.abs(start.x - gem.x) + Math.abs(start.y - gem.y)))
                    .collect(Collectors.toList()).get(0);
    }

    public void calculateHeuristica(Pair salida, List<Pair> gemas) {
        Pair posicion = this.pos;
        this.heuristica = 0;
        this.heuristica += 5*(9-gemas.size());
        if(gemas.size()<9){
    ...
    public NodoCompeticion applyAction(ACTIONS accion, List<Pair> gemas) {
        Pair pos_hijo = null;
